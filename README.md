...
- Web 端支持 JPG / PNG 单页图片、PDF 以及 ZIP 图片包下载
...
1. 在页面填写会议元数据（日期、地点、标题、主持人等），选择纸张样式并粘贴正文，也可直接导入 UTF-8 TXT 文件。
2. 右侧会显示纸张预览；点击缩略图可打开大图查看细节。
3. 如有需要，可在“高级设置”中切换 JPG / PNG 单页图片格式或设置随机种子。
4. 点击“开始生成”，页面会依次提示准备排版、计算分页、渲染笔迹和整理导出文件。
5. 生成完成后，可查看所有页面，并下载 PDF、全部图片 ZIP 或单独保存某一页。
6. 大图模式支持左右切换、按钮缩放、`Ctrl + 滚轮`缩放、拖拽平移以及键盘快捷键。
7. 未清空表单前，完整正文草稿会保留在当前浏览器标签页中，便于修改后重新生成。

### Web 端说明

- 生成结果文件保存在 `./output/` 目录。
- Web 端默认输出 JPG 单页图片，可在高级设置中切换为 PNG；每次生成都会同时尝试生成 PDF。
